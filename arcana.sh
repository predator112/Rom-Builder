...
curl -s https://api.telegram.org/bot$tg_token/sendMessage -d chat_id=$tg_id -d text="================================%0A$device Starting Building Rom%0A<b>     $rom_name</b>%0A(echo "${var_cache_report_config}")"
make bacon -j8
